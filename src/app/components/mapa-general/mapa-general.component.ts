...
export class MapaGeneralComponent implements OnInit {
  ...
  initMap(map: any) {
    ...
    service.nearbySearch(request8,
      (results: any, status: any) => {
        if (status == google.maps.places.PlacesServiceStatus.OK) {
          for (var i = 0; i < results.length; i++) {
            this.createMarkerN(results[i]);
          }
        }
      });
    service.nearbySearch(request9,
      (results: any, status: any) => {
        if (status == google.maps.places.PlacesServiceStatus.OK) {
          for (var i = 0; i < results.length; i++) {
            this.createMarkerN(results[i]);
    ...
    service.nearbySearch(request10,
      (results: any, status: any) => {
        if (status == google.maps.places.PlacesServiceStatus.OK) {
          for (var i = 0; i < results.length; i++) {
            this.createMarkerN(results[i]);
    ...
    service.nearbySearch(request11,
      (results: any, status: any) => {
        if (status == google.maps.places.PlacesServiceStatus.OK) {
          for (var i = 0; i < results.length; i++) {
            this.createMarkerN(results[i]);
    ...
    setTimeout(() => {
      service.nearbySearch(request12,
        (results: any, status: any) => {
          if (status == google.maps.places.PlacesServiceStatus.OK) {
            for (var i = 0; i < results.length; i++) {
              this.createMarkerP(results[i]);
            }
          }
        });

      service.nearbySearch(request13,
        (results: any, status: any) => {
          if (status == google.maps.places.PlacesServiceStatus.OK) {
            for (var i = 0; i < results.length; i++) {
              this.createMarkerP(results[i]);
      ...
      service.nearbySearch(request14,
        (results: any, status: any) => {
          if (status == google.maps.places.PlacesServiceStatus.OK) {
            for (var i = 0; i < results.length; i++) {
              this.createMarkerP(results[i]);
      ...
      service.nearbySearch(request15,
        (results: any, status: any) => {
          if (status == google.maps.places.PlacesServiceStatus.OK) {
            for (var i = 0; i < results.length; i++) {
              this.createMarkerP(results[i]);
  ...
  createMarkerV(place: google.maps.places.PlaceResult) {
    ...
    for (let location of this.locations) {
      if (location.lat() == place.geometry.location.lat() &&
        location.lng() == place.geometry.location.lng()) return
    }
    var infowindow: any;
    var map = this.map;
  ...
  createMarkerM(place: google.maps.places.PlaceResult) {
    if (!place.geometry || !place.geometry.location) return;
    if (place.name?.toLowerCase().includes('bomberos voluntarios')) return;
    for (let location of this.locations) {
      if (location.lat() == place.geometry.location.lat() &&
        location.lng() == place.geometry.location.lng()) return
    }
  ...
  createMarkerN(place: google.maps.places.PlaceResult) {
    if (!place.geometry || !place.geometry.location) return;
    for (let location of this.locations) {
      if (location.lat() == place.geometry.location.lat() &&
        location.lng() == place.geometry.location.lng()) return
    ...
    const marker = new google.maps.Marker({
      map,
      position: place.geometry.location,
      icon: '../../assets/img/nationalPolice.png',
      optimized: true,
      animation: google.maps.Animation.DROP
    });
    this.locations.push(place.geometry.location);
  ...
  createMarkerP(place: google.maps.places.PlaceResult) {
    if (!place.geometry || !place.geometry.location) return;
