...
export class PoliciasComponent implements OnInit {
  ...
  initMap(map: any) {
    ...
    service.nearbySearch(request4, (results: any, status: any) => {
      if (status == google.maps.places.PlacesServiceStatus.OK) {
        for (var i = 0; i < results.length; i++) {
    ...
    service.nearbySearch(request5, (results: any, status: any) => {
      if (status == google.maps.places.PlacesServiceStatus.OK) {
        for (var i = 0; i < results.length; i++) {
          this.createMarkerM(results[i]);
        }
    ...
    service.nearbySearch(request6, (results: any, status: any) => {
      if (status == google.maps.places.PlacesServiceStatus.OK) {
        for (var i = 0; i < results.length; i++) {
          this.createMarkerM(results[i]);
        }
    ...
    service.nearbySearch(request7, (results: any, status: any) => {
      if (status == google.maps.places.PlacesServiceStatus.OK) {
        for (var i = 0; i < results.length; i++) {
          this.createMarkerM(results[i]);
        }
